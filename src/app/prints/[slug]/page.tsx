...
export async function generateMetadata(
  ...
  return buildMetadata({
    title: `${print.title} ｜ 無料 PDF プリント（${print.subject}・${print.topic}）`,
...
function SolutionSection({ s }: { s: PrintSection }) {
  return (
    <section id={s.id} className="scroll-mt-24">
      <h3 className="mt-10 sm:mt-12 mb-3 font-serif text-[1.2rem] sm:text-[1.3rem] leading-snug text-ink-900">
        {s.heading}
      </h3>
...
export default async function PrintDetailPage({
  ...
  return (
    <>
      ...
      {/* HERO — title + meta + PDF DL (above the fold) */}
      <article>
        <Container className="py-10 sm:py-16">
          <div className="mx-auto max-w-3xl">
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-[10px] sm:text-[11px] tracking-[0.22em] sm:tracking-[0.28em] uppercase text-brand-deep">
              ...
              <time
                dateTime={print.publishedAt}
                className="text-ink-400 normal-case tracking-normal"
              >
                {formatDate(print.publishedAt)}
              </time>
            </div>

            <h1 className="mt-6 font-serif text-display-md sm:text-display-lg text-ink-900 leading-tight tracking-[-0.012em]">
              {print.title}
            </h1>
            <p className="mt-6 text-[15.5px] sm:text-[16px] leading-[1.95] text-ink-700">
              {print.lead}
            </p>
            ...
            <div className="mt-8 sm:mt-10 rounded-2xl border border-ink-900/[0.08] bg-gradient-to-br from-warm-bg/70 via-paper to-brand-bg/40 p-5 sm:p-7 shadow-card">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-[10px] tracking-[0.28em] uppercase text-warm-deep inline-flex items-center gap-1.5">
                    <Sparkles className="h-3 w-3" aria-hidden />
                    FREE PDF — REM 製プリント
                  </p>
                  <p className="mt-3 font-serif text-[1.05rem] sm:text-[1.15rem] leading-snug text-ink-900">
                    問題 + 解答 + 解説（{print.points} 点配点・印刷向け）
                  </p>
        ...
        <Container className="pb-10 sm:pb-14">
          <div className="mx-auto max-w-3xl print-content">
            <div className="rounded-2xl border border-ink-900/[0.08] bg-white/85 p-6 sm:p-9 shadow-soft">
              <div className="flex items-center justify-between border-b-2 border-warm/70 pb-3">
                <p className="font-mono text-[12px] tracking-[0.22em] text-ink-900">
                  REM
                </p>
              ...
              <h2 className="mt-5 sm:mt-7 font-serif text-[1.4rem] sm:text-[1.55rem] text-warm-deep tracking-[-0.01em]">
                問題
              </h2>
            ...
            {/* Inline CTA — note の有料プリントへ（同じ単元のもっと欲しい人向け） */}
            <aside className="mt-10 sm:mt-12 rounded-2xl border border-gold/30 bg-gradient-to-br from-gold-soft/40 via-white to-paper p-6 sm:p-8">
              <p className="flex items-center gap-2 text-[10px] tracking-[0.28em] uppercase text-gold-deep">
                <Sparkles className="h-3 w-3" aria-hidden />
                MORE PRINTS — note
              </p>
              <p className="mt-3 font-serif text-[1.05rem] sm:text-[1.1rem] leading-snug text-ink-900">
        ...
        <Container className="pb-14 sm:pb-20">
          <div className="mx-auto max-w-3xl print-content">
            <div className="rounded-2xl border border-ink-900/[0.08] bg-white/85 p-6 sm:p-9 shadow-soft">
              <h2 className="font-serif text-[1.4rem] sm:text-[1.55rem] text-warm-deep tracking-[-0.01em] border-b-2 border-warm/70 pb-3">
                解答・解説
              </h2>
            ...
            {/* Inline CTA — 体験授業へ（解答を見ても腑に落ちなかった人向け） */}
            <aside className="mt-10 sm:mt-12 rounded-2xl border border-brand/30 bg-gradient-to-br from-brand-bg via-white to-paper p-6 sm:p-8">
      ...
      {relatedPrints.length > 0 && (
        <Section eyebrow="RELATED PRINTS" title="関連する無料プリント" className="bg-paper-soft">
